...
output_file = "/Volumes/T9/nutrition5k_dataset/REMOVED_OUTLIERS/clip_embeddings_original_cleaned.pkl"
...
print("Embedding original images (cleaned dataset)...")

for _, row in tqdm(df.iterrows(), total=len(df), desc="Embedding"):
    dish_id = row["dish_id"].replace("dish_", "")
    img_path = os.path.join(image_dir, f"dish_{dish_id}.jpg")
...
print(f"Saved {len(results)} cleaned original embeddings to {output_file}")
